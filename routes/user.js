...
// 월별 회원가입 통계 라우트 추가
router.get('/membership-trends', authenticateToken, async (req, res) => {
  try {
    // 최근 12개월 데이터 가져오기
    const endDate = new Date();
    const startDate = new Date(endDate.getFullYear(), endDate.getMonth() - 11, 1); // 12개월 전
    ...
    // 모든 월에 대한 데이터 포인트 생성 (데이터가 없는 월도 0으로 표시)
    ...
    res.status(500).json({ message: '회원가입 추이 데이터 조회 중 오류가 발생했습니다.' });
  }
});
